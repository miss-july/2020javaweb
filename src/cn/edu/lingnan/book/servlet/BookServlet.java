...
public class BookServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		}else if("findBookByLike".equals(oper)) {
			//猜你喜欢
			findBookByLikeServlet(req,resp);
	...
	private void findBookByBidServlet(HttpServletRequest req, HttpServletResponse resp) 
		...
		//调用Service的findBookByBidService(press, pc)方法通过分类查询产品
	...
	private void findBookByCombinationServlet(HttpServletRequest req, HttpServletResponse resp) 
		...
		//调用Service的findBookByPressService(press, pc)方法通过分类查询产品
	...
	private void findBookByAuthorServlet(HttpServletRequest req, HttpServletResponse resp) 
		...
		String url=getUrl(req);
		String author=req.getParameter("author");
		//调用Service的findBookByAuthorService(author, pc)方法通过分类查询产品
		PageBean<Book> pageBean=null;
	...
	private void findBookByBnameServlet(HttpServletRequest req, HttpServletResponse resp)
		...
		String url=getUrl(req);
		String bname=req.getParameter("bname");
		//调用Service的findBookByBnameService(bname, pc)方法通过分类查询产品
	 ...
	 * 猜你喜欢
	 */
	private void findBookByLikeServlet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		//获得数据
		int uid=0;
		int pc=getPc(req);
		...
		//调用Service的findBookByLikeService(uid)方法查询产品
		PageBean<Book> pageBean=bookService.findBookByLikeService(uid,pc);
		pageBean.setUrl(url);
		req.setAttribute("pageBean", pageBean);
		//处理响应结果
		req.getRequestDispatcher("/jsps/book/list.jsp").forward(req, resp);
	...
	private void findBookByCategoryServlet(HttpServletRequest req, HttpServletResponse resp) 
		...
		//调用Service的findBookByCategoryService(cid, pc)方法通过分类查询产品
