...
public class UserServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		}else if("findAllUser".equals(oper)) {
			//调用查询所有订单的方法
			findAllUserServlet(req,resp);
		}
	...
	private void userQuitServlet(HttpServletRequest req, HttpServletResponse resp)
			throws IOException {
		User user=(User) req.getSession().getAttribute("sessionUser");
		userService.updateexittimeService(user.getName(),user.getPwd());
		//销毁Session
	...
	private void findUserByNameAndPwdServlet(HttpServletRequest req, HttpServletResponse resp) 
		...
		//响应处理结果
		if(user!=null) {
			userService.updatelogintimeService(name, pwd,req.getRemoteAddr());
			//将用户信息存储到session
			req.getSession().setAttribute("sessionUser", user);
	...
	private void findAllUserServlet(HttpServletRequest req, HttpServletResponse resp) 
			throws ServletException, IOException {
		//处理请求信息
		...
		if(userList!=null) {
			...
			req.getRequestDispatcher("/adminjsps/admin/user/user.jsp").forward(req, resp);
		}
		else {
			req.setAttribute("msg","还没有用户信息");
